...
export function KPIBar({ dataType, metrics, currentPrice, priceChange }: Props) {
  ...
  if (dataType === 'STOCK_OHLCV') {
    ...
    return (
      <div style={{ display: 'flex', overflowX: 'auto', borderBottom: '1px solid var(--c-line)' }}>
        {currentPrice != null && (
          <KPICard label="현재가" value={`${currentPrice.toLocaleString()}`} change={priceChange} />
        )}
        ...
        <KPICard label="최대 낙폭" value={`${fmtNum(m.mdd)}%`} colorType={m.mdd != null && m.mdd < -20 ? 'negative' : 'neutral'} />
        <KPICard label="변동성" value={`${fmtNum(m.volatility)}%`} colorType={m.volatility != null && m.volatility > 30 ? 'negative' : 'neutral'} />
        {m.rsi14 != null && (
